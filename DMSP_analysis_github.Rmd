...
Figure3 <- ggarrange(Mucusdmda, Mucusdddp, Holobiontdmda,
                     labels = c("(A)", "(B)", "(C)"),
                     ncol = 2, nrow = 2, 
                     common.legend = TRUE, legend = "bottom")
```
...
ratiolog <- ggscatter(nmr, x = "DMSP_nmol.ml", y = "dddP.dmdA_Ratio",
                add.params = list(color = "blue", fill = "lightgray"),
                )
ratiolog + stat_cor(aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~"),
                  method = "pearson", label.x = 0, label.y = 1.0, p.accuracy = 0.00001, r.accuracy = 0.001))
ratiolog + xscale("log10", .format = TRUE) + geom_point(stat = "identity", size = 9) # Add to log scale
